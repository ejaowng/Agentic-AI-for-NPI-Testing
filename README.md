...
(1) We used SWI-Prolog version 9.0.4, installed on an Ubuntu Linux virtual machine. The following commands were used to set up SWI-Prolog:
...
(2) An external built-in has been used for automation. The following commands are used to compile the external built-in:
...
(3) To run SWI-Prolog to monitor files generated in the predefined directory (the files are generated by the LLM):

cd ..

./src/swipl -s watch2.pl -g init_watch

(4) To run a prompt on the LLM:
